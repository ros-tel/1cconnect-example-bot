...
func SendFile(c *gin.Context, lineId uuid.UUID, userId uuid.UUID, fileName string, filepath string, comment *string, keyboard *[][]requests.KeyboardKey) (content []byte, err error) {
	cnf := c.MustGet("cnf").(*config.Conf)
